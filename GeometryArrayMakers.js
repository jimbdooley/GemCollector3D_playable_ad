...
function getFinishable() {
    ...
    for (let i = 0; i < 2*N; i++) {
        const th1 = 0.5 * PI - i * PI / N
        const th2 = 0.5 * PI - (i+1) * PI / N
        const xOfs = (i < N) ? 1 : -1
...
function getClickedCircle() {
    ...
    for (let i = 0; i < 2*N; i++) {
        ...
        const th1 = i * 2.0 * PI / N
        const th2 = (i+1) * 2.0 * PI / N
...
function getGearPosNormIndTex(bumps = 10) {
    ...
    for (let i = 0; i < bumps; i++) {
        points.push(Math.cos(2*PI*(i + toothSlope) / bumps))
        points.push(Math.sin(2*PI*(i + toothSlope) / bumps))
        points.push(Math.cos(2*PI*(i+GEAR_PEAK_R-toothSlope) / bumps))
        points.push(Math.sin(2*PI*(i+GEAR_PEAK_R-toothSlope) / bumps))
        points.push(GEAR_VALLEY_R*Math.cos(2*PI*(i+GEAR_PEAK_R) / bumps))
        points.push(GEAR_VALLEY_R*Math.sin(2*PI*(i+GEAR_PEAK_R) / bumps))
        points.push(GEAR_VALLEY_R*Math.cos(2*PI*(i+1) / bumps))
        points.push(GEAR_VALLEY_R*Math.sin(2*PI*(i+1) / bumps))
        innerPoints.push(innerR*Math.cos(2*PI*(i + 0.5) / bumps))
        innerPoints.push(innerR*Math.sin(2*PI*(i + 0.5) / bumps))
        innerPoints.push(innerR*Math.cos(2*PI*(i + 1) / bumps))
        innerPoints.push(innerR*Math.sin(2*PI*(i + 1) / bumps))
    }
    let units = 0
    for (let i = 0; i < bumps; i++) {
