...
function getStarPosNormInd() {
    const pos = [
        0, 1, 0, 1, 0, 0, STAR_Z, 1,
        STAR_VALLEY_TO_PEAK*Math.cos(PI*0.3), STAR_VALLEY_TO_PEAK*Math.sin(PI*0.3), 0, 1,
        0, 0, STAR_Z, 1, 0, 1, 0, 1,
        -STAR_VALLEY_TO_PEAK*Math.cos(PI*0.3), STAR_VALLEY_TO_PEAK*Math.sin(PI*0.3), 0, 1,
    ]
    for (let i = 1; i < 5; i++) {
...
function defineGemData(context) { 
    for (gemShape of GemShape.values()) {
        if (gemShape == GemShape.DONUT || gemShape == GemShape.ICO || gemShape == GemShape.MERKABA) continue
...
const PosNormIndTexs = {
    ...
    three: null,
    setupSecond() {
        if (this.crate != null) retur
        this.map_square = objProperToPosNormIndTex(assets["objs/map_square.obj"])
        this.map_hexagon = objProperToPosNormIndTex(assets["objs/map_hexagon.obj"])
        //this.gold_nugget = objProperToPosNormIndTex(assets["objs/gold_nugget.obj"])
    },
    setupFirst() {
...
function getSparklePosArr(x, y, z, P, r) {
    ...
    for (let i = 0; i < P*4; i += 4) {
        rtn[i] = x + r * Math.cos(i*0.5*PI/P)
        rtn[i+1] = y + r * Math.sin(i*0.5*PI/P)
        rtn[i+2] = z
    }
    return rtn
}
...
function getSparksPPPPTOfsInd(N=40, maxDt=0.15, uniformDarkness=false, maxDarkness=1,
    ...
    for (let n = 0; n < N; n++) {
        ...
        const angle = (2.0 * Math.random() * PI)
