...
function fitMap(map, region) {
    ...
    let maxScale = null
    if (isTablet) {
        maxScale = World.viewWidth > World.viewHeight ? 0.7 * map.mapType.maxScale : 0.95
    } else {
...
function setWorldXYZFromDeviceXY(target, devX, devY, worldZ) {
    const pct_dxy = (CAM_Z - worldZ) / CAM_Z
    const _dy = pct_dxy * World.dy
    const _dx = pct_dxy * World.dx
    target[0] = -_dx + 2.0 * _dx * devX / World.viewWidth
    target[1] = _dy - 2.0 * _dy * devY / World.viewHeight
    target[2] = worldZ
}
...
function xDistFromZAndDeviceRegion(z, deviceRegion) {
    setWorldXYZFromDeviceXY(World.dummyXYZ, deviceRegion[0], deviceRegion[1], z)
    const left = World.dummyXYZ[0]
    setWorldXYZFromDeviceXY(World.dummyXYZ, deviceRegion[0] + deviceRegion[2], deviceRegion[1], z)
    const right = World.dummyXYZ[0]
    return right - left
}

function yDistFromZAndDeviceRegion(z, deviceRegion) {
    setWorldXYZFromDeviceXY(World.dummyXYZ, deviceRegion[0], deviceRegion[1], z)
    const top = World.dummyXYZ[1]
    setWorldXYZFromDeviceXY(World.dummyXYZ, deviceRegion[0], deviceRegion[1] + deviceRegion[3], z)
    const bot = World.dummyXYZ[1]
    return top - bot
}
