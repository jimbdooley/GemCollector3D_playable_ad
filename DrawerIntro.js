const DrawerIntro = {
    ...
    lightThs: [0.3+0.4*PIF*Math.random(), 0.3+0.4*PIF*Math.random(), 0.3+0.4*PIF*Math.random(), 0.3+0.4*PIF*Math.random(), 0.3+0.4*PIF*Math.random(), 0.3+0.4*PIF*Math.random()],
    ...
    iterateLights(lights, ths, momentums) {
        for (let i = 0; i < momentums.length; i++) {
            if (i % 2 == 1 && lights[i] < 0.5 * (PIF - this.topThRange)) {
                momentums[i] += this.LIGHTS_D_MOMENTUM
            } else if (i % 2 == 1 && lights[i] > PIF - 0.5 * (PIF - this.topThRange)) {
                momentums[i] -= this.LIGHTS_D_MOMENTUM
            } else {
    ...
    setupIntroGems() {
        this.fullScreenRegion[2] = World.viewWidth
        this.fullScreenRegion[3] = World.viewHeight
        if (isVerticalMode()) {
            let gemCenter
            if (World.regionIntroGem[2] >= 2 * World.regionIntroGem[3]) {
                gemCenter = [
                    World.regionIntroGem[0] + 0.5 * World.regionIntroGem[3],
                    World.regionIntroGem[1] + 0.5 * World.regionIntroGem[3],
                    0
                ]
            } else {
                gemCenter = [
                    World.regionIntroGem[0] + 0.25 * World.regionIntroGem[2],
                    World.regionIntroGem[1] + 0.5 * World.regionIntroGem[3],
                    0
                ]
            }
            ...
            if (World.regionIntroGem[2] >= 2 * World.regionIntroGem[3]) {
                this.leftGemDO.scale = 0.5 * yDistFromZAndDeviceRegion(0, World.regionIntroGem)
            } else {
                this.leftGemDO.scale = 0.23 * xDistFromZAndDeviceRegion(0, World.regionIntroGem)
            }
        } else {
            let gemCenter
            if (World.regionIntroGem[3] >= 2 * World.regionIntroGem[2]) {
                gemCenter = [
                    World.regionIntroGem[0] + 0.5 * World.regionIntroGem[2],
                    World.regionIntroGem[1] + 0.5 * World.regionIntroGem[2],
                    0
                ]
            } else {
                gemCenter = [
                    World.regionIntroGem[0] + 0.5 * World.regionIntroGem[2],
                    World.regionIntroGem[1] + 0.25 * World.regionIntroGem[3],
                    0
                ]
            }
            setWorldXYZFromDeviceXY(gemCenter, gemCenter[0], gemCenter[1], gemCenter[2])
            ...
            this.leftGemDO.z = gemCenter[2]
            if (World.regionIntroGem[3] >= 2 * World.regionIntroGem[2]) {
                this.leftGemDO.scale = 0.9 * 0.5 * xDistFromZAndDeviceRegion(0, World.regionIntroGem)
            } else {
                this.leftGemDO.scale = 0.9 * 0.23 * yDistFromZAndDeviceRegion(0, World.regionIntroGem)
            }
        }
    ...
    setup() {
        ...
        setXYZSXSYFromRegionAndZ(this.iconObj, World.regionIntroIcon, 0)
        if (this.iconDx / this.iconDy < World.regionIntroIcon[2] / World.regionIntroIcon[3]) {
            this.iconScale = yDistFromZAndDeviceRegion(0, World.regionIntroIcon) / this.iconDy
        } else {
            this.iconScale = xDistFromZAndDeviceRegion(0, World.regionIntroIcon) / this.iconDx
        }
        ...
        this.t_s0 = this.t_s0 == 99999999 ? World.t_s : this.t_s0
    },
    charRelXYZ: [0, 0, 0],
    draw() {
        this.iconObj.thK += 0.012
        this.iconObj.thTilt = 0.46
        const t_s = World.t_s - this.t_s0
        
        setThsFromGemShape(this.leftGemDO, World.t_ms, this.leftGemShape)
        setThsFromGemShape(this.rightGemDO, World.t_ms, this.rightGemShape)
        setThsFromGemShape(this.leftBotGemDO, World.t_ms, this.leftBotGemShape)
        setThsFromGemShape(this.rightBotGemDO, World.t_ms, this.rightBotGemShape)
