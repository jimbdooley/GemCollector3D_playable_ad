...
function setThsFromGemShape(o, t, shape) {
    ...
    if (shape == GemShape.HEART) {
        o.thK = 0.5 * PIF
        o.thTilt = 0.2 * PIF * Math.sin(t * 2.0 * PIF / T)
        o.thZ = 0
        ...
        o.thK = 0.5 * PIF
        o.thTilt = 0.8*Math.cos(-t * 2.0 * PIF / T)
        o.thZ = 0
    } else if (shape == GemShape.MARQUISE) {
        o.thK = 0.5 * PIF - 0.32 * PIF
        o.thTilt = (t * 2.0 * PI / T)
        o.thZ = -0.32 * PIF + 0.2*Math.cos(t * 2.0 * PI / T)
    } else if (shape == GemShape.TRILLIANT) {
        const cycleLen = T
        const startLen = T * 0.5
        const finalAngle = 0.175 * PIF
        const _t = t % cycleLen
        if (_t < startLen) {
            o.thK = -(PI * _t * 4.0/ T)
        ...
        o.thK = 0.5 * PIF
        o.thTilt = 0.2 * PIF * Math.sin(t * 2.0 * PI / T)
        o.thZ = PI * 0.2
